    ...
    override fun onEnable() {
        ...
        GlobalEventChannel.filter { event: Event ->
            ...
            if (!gameData.containsKey(event.group.id)) {
                ...
                event.group.sendMessage("你是第一个加入游戏的玩家,请等待其他人加入游戏...\n你现在可以向待选池中添加关键词,格式为:/添加nk 关键词")
                return@subscribeAlways
            }
            //重复加入
            if (gameData[event.group.id]?.containsKey(event.sender.id) == true) {
                event.group.sendMessage("你已经加入游戏了哦~")
                return@subscribeAlways
            }
            gameData[event.group.id]  = mutableMapOf(event.sender.id to "")
            event.group.sendMessage("你已经成功加入了本局游戏√")
        }
        //添加关键词
        GlobalEventChannel.filter { event: Event ->
            event is GroupMessageEvent && event.message.contentToString().startsWith("/添加nk")
                    && gameData[event.group.id]?.containsKey(event.sender.id) == true
        }.subscribeAlways<GroupMessageEvent> { event ->
            val messageOri = event.message.contentToString().substring(4)
            val keyWordsList = messageOri.split("%s".toRegex())
            keyWordsList.forEach(){
                if (keyWords[event.group.id]?.containsKey(it) == false) {
                    keyWords[event.group.id]?.put(it,event.sender.nameCard)
                }
            }
            event.group.sendMessage("关键词添加成功√")
        }
        //开始游戏
        GlobalEventChannel.filter { event: Event ->
        ...
        }.subscribeAlways<GroupMessageEvent> { event:GroupMessageEvent ->
